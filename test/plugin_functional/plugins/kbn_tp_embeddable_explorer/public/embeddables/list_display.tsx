...
export class ListDisplay extends Component<Props, State> {
  ...
  private unsubscribeOutputChanges?: () => void;
  ...
  public async componentDidMount() {
    ...
    this.unsubscribeInputChanges = this.props.container.subscribeToInputChanges(input => {
      Object.values(input.panels).forEach(async panelState => {
        ...
        if (this.state.embeddables[panelState.embeddableId] === undefined) {
          ...
          if (this.mounted && node !== null && embeddable) {
            embeddable.renderWithChrome(node);
    ...
    this.unsubscribeOutputChanges = this.props.container.subscribeToOutputChanges(output => {
  ...
  public componentWillUnmount() {
    ...
    if (this.unsubscribeOutputChanges) {
      this.unsubscribeOutputChanges();
    }
